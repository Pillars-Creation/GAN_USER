...
from model import AttentionalFactorizationMachineModel, DeepCrossNetworkModel, MWUF, MetaE, CVAR, GAR
...
def get_args():
    ...
    parser.add_argument('--features_id', default='user_id')
...
def get_loaders(name, datahub_path, device, bsz, shuffle):
    path = os.path.join(datahub_path, name, "{}_data_user.pkl".format(name))
    if name == 'movielens1M':
    ...
    for epoch_i in range(1, epoch + 1):
        ...
        for i, (features, label) in enumerate(data_loader):
            if random.random() < 0.1:
                bsz = label.shape[0]
                item_emb = model.emb_layer[args.features_id]
                origin_item_emb = item_emb(features[args.features_id]).squeeze(1)
...
def mwuf(model,
    ...
    mwuf_model = MWUF(model,
                      item_features=dataloaders.user_features,
...
def gar(model,
    ...
    save_path = os.path.join(save_dir, 'model.pth')
    train_base = dataloaders['train_base']
    GAR_model = GAR(model, warm_features=dataloaders.user_features, device=device).to(device)
    GAR_model.init_generator()

    def inverse_sigmoid(x):
        # x -> 1, nan
        return torch.log(x / (1 - x))

    def warm_up(data_loader, epochs, log_interval=10, logger=False):
        GAR_model.train()
        criterion = torch.nn.BCELoss()
        ...
        #         GAR_model.model.only_optimize_itemid()
        #         optimizer_d = torch.optim.Adam(params=model.item_id_name.parameters(), \
        #                                        lr=lr, weight_decay=weight_decay)
        #         optimizer_d = torch.optim.Adam(params=model.parameters(), \
        #                                        lr=lr, weight_decay=weight_decay)

        #         GAR_model.optimize_generator()
        optimizer_g = torch.optim.Adam(params=GAR_model.itemid_generator.parameters(), \
                                       lr=lr, weight_decay=weight_decay)
        # RuntimeError: one of the variables needed for gradient computation has been modified by an inplace operation, solve: https://github.com/pytorch/pytorch/issues/39141
        for epoch_i in range(epochs):
            ...
            total_iters = len(data_loader)
            for i, (features, label) in enumerate(data_loader):
                target_real, target_fake, item_id_emb, new_item_id_emb = GAR_model(features)
                loss_d = beta * criterion(torch.sigmoid(target_fake), label.float()) + (1 - beta) * criterion(
                    label * torch.sigmoid(target_real - target_fake), label.float())
                #                 loss_d = beta * criterion(target_fake, label.float()) \
                #                          + (1 - beta) * criterion(label * torch.sigmoid(inverse_sigmoid(target_real) - inverse_sigmoid(target_fake)), label.float())

                loss_d = (-label * torch.log(torch.sigmoid(target_fake)) - (1 - label) * (
                    torch.log(1 - torch.sigmoid(target_fake)))).mean() * beta - (
                                     torch.log(torch.sigmoid(target_real - target_fake)) * label).mean() * (1 - beta)
                model.zero_grad()
                loss_d.backward(retain_graph=True)
                optimizer_d.step()

                #                 print(torch.isnan().any())
                target_real, target_fake, item_id_emb, new_item_id_emb = GAR_model(features)
                loss_g = - (torch.log(torch.sigmoid(target_fake - target_real)) * label).mean()
                #                 loss_g = criterion(label * torch.sigmoid(inverse_sigmoid(target_fake) - inverse_sigmoid(target_real)), label.float())
                #                 loss_g = -(1 - alpha) * torch.sum(torch.log(torch.sigmoid(target_fake - target_real)) * label) / torch.norm(label.float(), 0)
            ...
            if (i + 1) % log_interval == 0:
                print("     discriminator iters {}/{} loss: {:.4f}".format(i + 1, total_iters + 1,
                                                                           total_d_loss / log_interval), end='\r')
                print("     generator iters {}/{} loss: {:.4f}".format(i + 1, total_iters + 1,
                                                                       total_g_loss / log_interval), end='\r')
                total_d_loss = 0
                total_g_loss = 0

            print("discriminator Epoch {}/{} loss: {:.4f}".format(epoch_i, epoch, epoch_d_loss / total_iters), " " * 20)
            print("generator Epoch {}/{} loss: {:.4f}".format(epoch_i, epoch, epoch_g_loss / total_iters), " " * 20)

        # warm-up item id embedding
        train_a = dataloaders['train_warm_a']
        for (features, label) in train_a:
            origin_item_id_emb = GAR_model.model.emb_layer[GAR_model.item_id_name].weight.data
            warm_item_id_emb = GAR_model.warm_item_id(features)
            indexes = features[GAR_model.item_id_name].squeeze()
            origin_item_id_emb[indexes,] = warm_item_id_emb

    warm_up(train_base, epochs=1, logger=True)
    ...
    for i, train_s in enumerate(dataset_list):
        print("#" * 10, dataset_list[i], '#' * 10)
        train_s = dataset_list[i]
        auc, f1 = test(GAR_model.model, dataloaders['test'], device)
        auc_list.append(auc.item())
        print("[gar] evaluate on [test dataset] auc: {:.4f}, F1 score: {:.4f}".format(auc, f1))
        if i < len(dataset_list) - 1:
            #             GAR_model.model.only_optimize_itemid()
            #             train(GAR_model.model, dataloaders[train_s], device, epoch, lr, weight_decay, save_path)
            warm_up(dataloaders[train_s], epochs=epoch, logger=False)
    print("*" * 20, "gar", "*" * 20)
    return auc_list
...
def run(model, dataloaders, args, model_name, warm):
    if warm == 'base':
        ...
        auc_list = gar(model, dataloaders, model_name, args.epoch, args.lr, args.weight_decay, args.device, args.save_dir)
    return auc_list
