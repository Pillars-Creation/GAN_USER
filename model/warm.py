...
class DropoutNet(nn.Module):
    ...
    def __init__(self, model: nn.Module, device, item_id_name='user_id'):
...
class GAR(nn.Module):
    ...
    def optimize_discriminator(self):
        for name, param in self.named_parameters():
            if 'itemid_generator' not in name:
    ...
    def forward(self, x_dict):
        ...
        new_item_id_emb = self.warm_item_id(x_dict)
        target_real = self.model.forward_with_item_id_emb_without_sigmoid(item_id_emb, x_dict)
        target_fake = self.model.forward_with_item_id_emb_without_sigmoid(new_item_id_emb, x_dict)
#         target_real = self.model.forward_with_item_id_emb(item_id_emb, x_dict)
#         target_fake = self.model.forward_with_item_id_emb(new_item_id_emb, x_dict)
        return target_real, target_fake, item_id_emb, new_item_id_emb

class CVAEGAR(nn.Module):
...
#                 emb = self.warmup_emb_layer[name](x).sum(dim=1, keepdims=True).squeeze() / torch.where((x != 0).sum(dim=1).view([-1, 1]) != 0, (x != 0).sum(dim=1).view([-1, 1]), 1)
#                 emb = self.warmup_emb_layer[name](x).sum(dim=1, keepdims=True).squeeze() / torch.clamp((x != 0).sum(dim=1).view([-1, 1]), min=1)
    ...
    def forward(self, x_dict):
        item_ids = x_dict[self.item_id_name]
        item_id_emb = self.origin_item_emb(item_ids).squeeze()
        new_item_id_emb, reg_term, recon_term = self.warm_item_id(x_dict)
        target_real = self.model.forward_with_item_id_emb(item_id_emb, x_dict)
        target_fake = self.model.forward_with_item_id_emb(new_item_id_emb, x_dict)
        return target_real, target_fake, item_id_emb, new_item_id_emb, reg_term, recon_term
...
class Immature(nn.Module):
    ...
    def init_immature(self):
        for name, param in self.named_parameters():
            if 'itemid_generator' in name:
                torch.nn.init.uniform_(param, -0.01, 0.01)

    def optimize_immature(self):
        for name, param in self.named_parameters():
            if 'itemid_generator' in name:
                param.requires_grad_(True)
            else:
...
class CVARL(nn.Module):
    ...
    def build(self,
        ...
        self.warmup_emb_layer["warmup_item_id_young"] = nn.Embedding(3953,16)

        self.item_id_emb_young = torch.nn.Embedding(3953,16) #取原始itemid_shape,emb_size, 幼小状态
        self.mean_encoder = nn.Linear(emb_dim, 16)
        self.log_v_encoder = nn.Linear(emb_dim, 16)
        self.mean_encoder_p = nn.Linear(emb_dim, 16)
        self.log_v_encoder_p = nn.Linear(emb_dim, 16)
        self.decoder = nn.Linear(17, 16)
        return
    ...
    def warm_item_id(self, x_dict):
        ...
        item_id_emb = self.origin_item_emb(item_ids).squeeze()
        # get embedding of item features
        item_id_emb_young = self.warmup_emb_layer["warmup_item_id_young"](item_ids).squeeze()
        '''
        item_embs = []
        for item_f in self.item_features: 
            type = self.model.description[item_f][1]
...
#                 emb = self.warmup_emb_layer[name](x).sum(dim=1, keepdims=True).squeeze() / torch.clamp((x != 0).sum(dim=1).view([-1, 1]), min=1)
        ...
        sideinfo_emb = torch.concat(item_embs, dim=1)
        '''
        sideinfo_emb = item_id_emb_young
        mean = self.mean_encoder(item_id_emb)
        log_v = self.log_v_encoder(item_id_emb)
        mean_p = self.mean_encoder_p(sideinfo_emb)
        log_v_p = self.log_v_encoder_p(sideinfo_emb)
