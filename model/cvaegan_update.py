...
# 预估的时候decode加入特征
        ...
        # else:
        #     freq = x_dict[self.c_feature]
        freq = x_dict['clk_seq'][:, [0]]
        # freq = self.model.get_seq_emb_sum('clk_seq', x_dict).mean(dim=1, keepdims=True)
        # freq = self.model.get_seq_emb_sum('genres', x_dict)
        freq = x_dict['count']
    ...
    def forward(self, x_dict):
        ...
        self.c_feature_emb = self.model.get_seq_emb_sum(self.c_feature, x_dict)
...
class LAVAR(nn.Module):
    ...
    def build(self,
        ...
        self.model = model
        self.device = device
        assert item_id_name in model.user_id_name, \
            "illegal item id name: {}".format(item_id_name)
        self.item_id_name = item_id_name
        self.item_features = []
        self.output_emb_size = 0
    ...
    def warm_item_id(self, x_dict):
        ...
        condition = torch.reshape(x_dict['count'], (-1,))
        print(condition)
        item_id_emb_young = torch.no_grad(
            item_id_emb_young)  # torch.where(condition>1000, torch.no_grad(item_id_emb_young), item_id_emb_young)
