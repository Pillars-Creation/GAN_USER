...
class FactorizationMachineModel(torch.nn.Module):
    ...
    def __init__(self, description, embed_dim, item_id_name='item_id'):
        super().__init__()
        ...
        self.build(embed_dim)
    
    def build(self, embed_dim):
        ...
        for name, (size, type) in self.description.items():
            if type == 'spr':
                ...
                self.emb_layer[name] = torch.nn.Embedding(size, embed_dim, padding_idx=0)
            elif type == 'label':
                pass
            else:
    ...
    def forward_with_item_id_emb(self, item_id_emb, x_dict):
        ...
        for name, (_, type) in self.description.items():
            ...
            if type == 'spr':
                ...
                embs.append(self.emb_layer[name](x).sum(dim=1, keepdims=True) / torch.where((x != 0).sum(dim=1).view([-1, 1, 1]) != 0, (x != 0).sum(dim=1).view([-1, 1, 1]), 1))
            else:
        ...
        res = (linear_part + self.fm(emb)).squeeze(dim=1)
